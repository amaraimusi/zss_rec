 ...
 * 
 * @note
 * 心得カテゴリー画面では心得カテゴリー一覧を検索閲覧、編集など多くのことができます。
 * 
...
class KlCategoryController extends CrudBaseController {
	...
	/// 名称コード
	public $name = 'KlCategory';
	
	/// 使用しているモデル
	public $uses = array('KlCategory','CrudBase');
	
	/// オリジナルヘルパーの登録
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='KlCategory.sort_no';
	...
	public function index() {
        ...
        // CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('KlCategory');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->KlCategory->findData2($crudBaseData);
	...
	public function detail() {
		...
		$res=$this->edit_before('KlCategory');
		$ent=$res['ent'];
	

		$this->set(array(
				'title_for_layout'=>'心得カテゴリー・詳細',
				'ent'=>$ent,
		));
	 ...
	 */
	public function reg(){
		$res=$this->reg_before('KlCategory');
		$ent=$res['ent'];
		
		$regMsg="<p id='reg_msg'>更新しました。</p>";

		//オリジナルバリデーション■■■□□□■■■□□□■■■□□□
		//$xFlg=$this->validKlCategory();
		$xFlg=true;
		if($xFlg==false){
		...
		//★DB保存
		$this->KlCategory->begin();//トランザクション開始
		$ent=$this->KlCategory->saveEntity($ent);//登録
		$this->KlCategory->commit();//コミット

		$this->set(array(
				'title_for_layout'=>'心得カテゴリー・登録完了',
				'ent'=>$ent,
				'regMsg'=>$regMsg,
		));
	...
	public function ajax_reg(){
		...
		if(!empty($_FILES["upload_file"])){
			...
			$ent['kl_category_fn'] = $upload_file;
		}
		...
		// エンティティをDB保存
		$this->KlCategory->begin();
		$ent = $this->KlCategory->saveEntity($ent,$regParam);
		$this->KlCategory->commit();//コミット
	...
	public function ajax_delete(){
		...
		$this->KlCategory->begin();
		if($eliminate_flg == 0){
			$ent = $this->KlCategory->saveEntity($ent,$regParam); // 更新
		}else{
		    $this->KlCategory->delete($ent['id']); // 削除
		}
		$this->KlCategory->commit();//コミット
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->KlCategory->begin();
		$this->KlCategory->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
		$this->KlCategory->commit();
	...
	public function csv_fu(){
		...
		$this->csv_fu_base($this->KlCategory,array('id','kl_category_val','kl_category_name','kl_category_date','kl_category_group','kl_category_dt','note','sort_no'));
	...
	private function getDataForDownload(){
        ...
        //セッションから検索条件情報を取得
        $kjs=$this->Session->read('kl_category_kjs');
        
        // セッションからページネーション情報を取得
        $pages = $this->Session->read('kl_category_pages');
		...
		//DBからデータ取得
	   $data=$this->KlCategory->findData($kjs,$page_no,$row_limit,$sort_field,$sort_desc);
		if(empty($data)){
			return array();
		}
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_kl_category_name','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_kl_category_name'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => 'カテゴリ名は64文字以内で入力してください',
								'allowEmpty' => true
						),
				),
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'KlCategory.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),
			'kl_category_name'=>array(
					'name'=>'カテゴリ名',
					'row_order'=>'KlCategory.kl_category_name',
					'clm_show'=>1,
			),
			'category_code'=>array(
					'name'=>'カテゴリコード',
					'row_order'=>'KlCategory.category_code',
					'clm_show'=>1,
			),
			'sort_no'=>array(
					'name'=>'順番',
