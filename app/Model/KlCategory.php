...
/**
 * 心得カテゴリーのCakePHPモデルクラス
 *
 * @date 2015-9-16 | 2018-4-24 複製したとき、順番はそのまま
 * @version 3.0.3
 *
 */
class KlCategory extends AppModel {

	public $name='KlCategory';
	
	// 関連付けるテーブル CBBXS-1040
	public $useTable = 'kl_categorys';
	 ...
	 * 心得カテゴリーエンティティを取得
	 *
	 * 心得カテゴリーテーブルからidに紐づくエンティティを取得します。
	 *
	 * @param int $id 心得カテゴリーID
	 * @return array 心得カテゴリーエンティティ
	 */
	...
	/**
	 * 心得カテゴリー画面の一覧に表示するデータを、心得カテゴリーテーブルから取得します。
	 * 
	 * @note
	 * 検索条件、ページ番号、表示件数、ソート情報からDB（心得カテゴリーテーブル）を検索し、
	 * 一覧に表示するデータを取得します。
	 * 
	...
	private function dumpSql($option){
		...
		$option['table']=$dbo->fullTableName($this->KlCategory);
		$option['alias']='KlCategory';
		
		$query = $dbo->buildStatement($option,$this->KlCategory);
	...
	private function createKjConditions($kjs){
		...
		if(!empty($kjs['kj_id']) || $kjs['kj_id'] ==='0' || $kjs['kj_id'] ===0){
			$cnds[]="KlCategory.id = {$kjs['kj_id']}";
		}
		if(!empty($kjs['kj_kl_category_name'])){
			$cnds[]="KlCategory.kl_category_name LIKE '%{$kjs['kj_kl_category_name']}%'";
		}
		if(!empty($kjs['kj_category_code'])){
			$cnds[]="KlCategory.category_code LIKE '%{$kjs['kj_category_code']}%'";
		}
		if(!empty($kjs['kj_sort_no']) || $kjs['kj_sort_no'] ==='0' || $kjs['kj_sort_no'] ===0){
			$cnds[]="KlCategory.sort_no = {$kjs['kj_sort_no']}";
		}
		$kj_delete_flg = $kjs['kj_delete_flg'];
		if(!empty($kjs['kj_delete_flg']) || $kjs['kj_delete_flg'] ==='0' || $kjs['kj_delete_flg'] ===0){
			if($kjs['kj_delete_flg'] != -1){
			   $cnds[]="KlCategory.delete_flg = {$kjs['kj_delete_flg']}";
			}
		}
		if(!empty($kjs['kj_update_user'])){
			$cnds[]="KlCategory.update_user LIKE '%{$kjs['kj_update_user']}%'";
		}
		if(!empty($kjs['kj_ip_addr'])){
			$cnds[]="KlCategory.ip_addr LIKE '%{$kjs['kj_ip_addr']}%'";
		}
		if(!empty($kjs['kj_created'])){
			$kj_created=$kjs['kj_created'].' 00:00:00';
			$cnds[]="KlCategory.created >= '{$kj_created}'";
		}
		if(!empty($kjs['kj_modified'])){
			$kj_modified=$kjs['kj_modified'].' 00:00:00';
			$cnds[]="KlCategory.modified >= '{$kj_modified}'";
		}
	 ...
	 * エンティティをDB保存
	 *
	 * 心得カテゴリーエンティティを心得カテゴリーテーブルに保存します。
	 *
	 * @param array $ent 心得カテゴリーエンティティ
	 * @param array $option オプション
	 *  - form_type フォーム種別  new_inp:新規入力 , copy:複製 , edit:編集
	 *  - ni_tr_place 新規入力追加場所フラグ 0:末尾 , 1:先頭
	 * @return array 心得カテゴリーエンティティ（saveメソッドのレスポンス）
	 */
	public function saveEntity($ent,$option=array()){
		...
		$ent = $this->find('first',
				array(
						'conditions' => "id={$ent['KlCategory']['id']}"
				));
