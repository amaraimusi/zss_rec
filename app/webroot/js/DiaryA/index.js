...
function init(){
	...
	// 日付カレンダーのセット
	// CBBXS-1030
	$("#new_inp_diary_date").datepicker({dateFormat:'yy-mm-dd'});
	$("#edit_diary_date").datepicker({dateFormat:'yy-mm-dd'});

	// CBBXE
	
	// ■■■□□□■■■□□□■■■□□□■■■
//	// CSVインポートの初期化  <CrudBase/index.js>
//	initCsvImportFu('diary_a/csv_fu');
...
function tableTransform(mode_no){
	...
	if(mode_no == 1){
		jQuery("#diary_a_tbl").addClass('table_transform');
		jQuery("#table_transform_tbl_mode").show();
		jQuery("#table_transform_div_mode").hide();
	}else{
		jQuery("#diary_a_tbl").removeClass('table_transform');
