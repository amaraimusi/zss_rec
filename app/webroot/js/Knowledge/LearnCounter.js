...
class LearnCounter{
	...
	constructor(){
		...
		this._setLearnedToTbl(this.data);
	 ...
	 */
	_setLearnedToTbl(data){
		
		var tbl_slt = '#knowledge_tbl';
		var tbl = jQuery(tbl_slt);
		
		jQuery("#knowledge_tbl tbody tr").each((i,tr)=>{
			tr = jQuery(tr);
			...
			if(ent.learned == false){
				td1.find('.learn_btn').hide(); // 覚えボタンを隠す
				
				// 約残日を表示する
				var learned_span = td1.find('.learned');
